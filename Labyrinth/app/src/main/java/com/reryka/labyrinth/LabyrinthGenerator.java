...
public class LabyrinthGenerator {
        ...
        for (int y = 0; y < verticalBlockCount; y++) {
            for (int x = 0; x < horizontalBlockCount; x++) {
                if (map[y][x] == INNER_WALL) {
                    List<Direction> directionList = new ArrayList<>(Arrays.asList(Direction.LEFT, Direction.RIGHT, Direction.BOTTOM));

                    if (y == 1) {
                        directionList = new ArrayList<>(Arrays.asList(Direction.TOP, Direction.LEFT, Direction.RIGHT, Direction.BOTTOM));
                    }
    ...
    private static void setHoles(int holeCount, Random rand, int verticalBlockCount, int horizontalBlockCount, int[][] map) {
        do {
