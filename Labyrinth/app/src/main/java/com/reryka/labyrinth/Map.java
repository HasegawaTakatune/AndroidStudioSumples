...
public class Map implements Ball.OnMoveListener {
    ...
    public Map(int width, int height, int blockSize, int stageSeed, LabyrinthView.EventCallback eventCallback) {
    ...
    private boolean canMove(Rect movedRect) {
        ...
        if (placeHorizontalBlock != horizontalBlock || placeVerticalBlock != verticalBlock) {
        ...
        for (int y = 0; y < yLength; y++) {
            ...
            for (int x = 0; x < xLength; x++) {
                ...
                if (block.type == Block.TYPE_WALL && Rect.intersects(block.rect, movedRect)) {
                    return false;
                } else if (block.type == Block.TYPE_GOAL && block.rect.contains(movedRect.centerX(), movedRect.centerY())) {
                    eventCallback.onGoal();
                    return true;
                } else if (block.type == Block.TYPE_HOLE && block.rect.contains(movedRect.centerX(), movedRect.centerY())) {
